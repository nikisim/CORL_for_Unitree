...
class Config:
    ...
    # training params
    dataset_name: str = "FetchSlideDense-v2"
    batch_size: int = 2048
    num_epochs: int = 6000
...
class ReplayBuffer:
    ...
    def create_from_d4rl(
        ...
        dataset = np.load('data/FetchSlideDense.npy', allow_pickle=True).item()
...
def make_env(env_name: str, seed: int) -> gym.Env:
    ...
    env = gym.wrappers.RecordVideo(env, f"videos/FetchSlide_more_lay_actor_ln_2048b", episode_trigger = lambda x: x % 800 == 0)
...
def main(config: Config):
    ...
    data = np.load('data/FetchSlideDense.npy', allow_pickle=True).item()
