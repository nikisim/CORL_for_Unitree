...
class Config:
    ...
    # training params
    dataset_name: str = "FetchPushDense-v2"
    batch_size: int = 1024
    num_epochs: int = 6000
...
class ReplayBuffer:
    ...
    def create_from_d4rl(
        ...
        dataset = np.load('data/FetchPushDense.npy', allow_pickle=True).item()
...
def make_env(env_name: str, seed: int) -> gym.Env:
    env = gym.make('FetchPushDense-v2', render_mode='rgb_array')
    env = gym.wrappers.RecordVideo(env, f"videos/FetchPush1", episode_trigger = lambda x: x % 700 == 0)
    # wandb.log({'video': wandb.Video("videos/FetchSlide4")})
    # env.seed(seed)
...
def evaluate(
    ...
    print(f"{sum(success)} Episodes out of {len(success)}")
...
def main(config: Config):
    ...
    data = np.load('data/FetchPushDense.npy', allow_pickle=True).item()
    ...
    for epoch in trange(config.num_epochs, desc="ReBRAC Epochs"):
        ...
        if epoch % config.eval_every == 0 or epoch == config.num_epochs - 1:
            ...
            normalized_score = get_normalized_score(eval_returns, REF_MIN_SCORE, REF_MAX_SCORE) * 100.0
            wandb.log(
                {
                    "epoch": epoch,
